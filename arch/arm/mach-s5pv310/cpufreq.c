		...
		unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
		ret = cpufreq_driver_target(cpufreq_cpu_get(cpu),
		s5pv310_freq_table[L5].frequency, DISABLE_FURTHER_CPUFREQ);
		if (WARN_ON(ret < 0))
			return NOTIFY_BAD;
#ifdef CONFIG_S5PV310_BUSFREQ
	...
	case PM_POST_SUSPEND:
		...
		ret = cpufreq_driver_target(cpufreq_cpu_get(cpu),
		s5pv310_freq_table[L5].frequency, ENABLE_FURTHER_CPUFREQ);
#ifdef CONFIG_S5PV310_BUSFREQ
...
static int s5pv310_asv_table_update(void)
{
	...
	for (i = 0; i < last_level; i++) {
		...
		/* Minimum Voltage */
		if (s5pv310_volt_table[i].arm_volt < 850000)
			s5pv310_volt_table[i].arm_volt = 850000;
